...
export const SavedListingsProvider = ({ children, userId }) => {
    ...
    const saveListing = async (listing) => {
        try {
            const response = await axios.post(`http://localhost:3000/users/1/saved-listings`, { listing });
            setSavedListings(response.data);
        } catch (error) {
    ...
    const removeListing = async (listingId) => {
        try {
            ...
            const response = await axios.delete(`http://localhost:3000/users/1/saved-listings/${listingId}`);
            setSavedListings(response.data);
        } catch (error) {
