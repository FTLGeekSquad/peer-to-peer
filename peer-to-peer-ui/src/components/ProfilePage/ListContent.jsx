...
const ListContent = () => {
	...
	const fetchListings = async() => {
		try {
			const response = await axios.get(`https://peer-to-peer-59rz.onrender.com/listings/all-listings/${userData.userId}`)
 ...
 const deleteUser = async (userId) => {
	try {
		const response = await axios.delete(
			`https://peer-to-peer-59rz.onrender.com/users/${userData.userId}`
		);
		handleLogout();
	} catch (error) {
...
const handleUserConfirmedDelete = async () => {
	...
	const handleEdit = async (event) => {
        ...
        try {
            await axios.put(`https://peer-to-peer-59rz.onrender.com/users/${userData.userId}`, {
...
const deleteListing = async (listingId) => {
	try {
		...
		const response = await axios.delete(
			`https://peer-to-peer-59rz.onrender.com/listings/${listingId}`
		);
...
const handleEditListing = async (e) => {
	...
	try {
		const response = await axios.put(
			`https://peer-to-peer-59rz.onrender.com/listings/${selectedEquipment.listingId}`,
			updatedListingData
		);
